...
void log_router_cycles(uint64_t cycles)
{
    if (router_cycle_fp == NULL) {
        ...
        char fname[128];
        int n = snprintf(fname, sizeof(fname), "/tmp/router_cycles_lcore%u.csv", lcore);
        (void)n;
        router_cycle_fp = fopen(fname, "a");
        if (router_cycle_fp == NULL) {
            /* 失敗しても処理は続けられるようにする */
            perror("fopen router_cycles file");
            return;
        }
        /* 別プロセスで集計しやすいようにヘッダは書かない（単純な数値の列にする） */
    }
...
//  printf("Initializing nodes...\n");
   ...
   //  uint32_t sig_size = 0;//sizeof(sig);
   ...
   //  // --- SID 生成 ---
   //  size_t sid_len;
   //  unsigned char *sid_data = concat2(kS_pub, PUB_LEN, uc_sig, sig_size, &sid_len);
   // //  print_hex("SID input=H(kC||sig)", sid, sid_len);
   // hash_sid(sid_data, sid_len, sid);
   ...
   //  sign_data(me->sk, m, m_len, t, &tau_len);
   //  free(m);
    // 受信側で τ_ROUTERSを生成(復路の検証用&通報用 本来は state から取得)
   ...
   // printf("R3 state set\n");
   // state_set(me, sid, prev_addr, next_addr, nnext_addr, t, tau_len);
   //  print_hex("τ (Receiver)", t, tau_len);
    for (int kk = 0; ; ++kk)
   {
      ...
      //   printf ("Received %zu packets on port %d\n", nb_rx, rxport);
        if (nb_rx > 0) {
            if (n_all_rx >= 10000) {
               printf("Reached 10000 packets, stopping...\n");
                break;
            }
            for (int i = 0; i < nb_rx; i++)
            {
               ...
               // print_hex("Encrypted data", ctr, wire_len);
               ...
               // // //パケット長チェック
               // if (rx_pkt_mbuf[i]->pkt_len < (sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) + SID_LEN + CID_LEN + 2)) {
               ...
               // router_handle_data_trans(rx_pkt_mbuf[i], nodes);
                ...
                router_cycles += (end - start);
               //  printf("%" PRIu64 "\n", end-start);
               //  log_router_cycles(end - start);
...
int main(int argc, char *argv[])
{
   ...
   */
    size_t wanted_data_room = FRAME_SIZE + RTE_PKTMBUF_HEADROOM;//1152
   ...
   pkt_pool0 = rte_pktmbuf_pool_create("PKT_POOL0", // mempool name
                                       10000,//NUM_MBUFS,   // number_of_elements *2^q -1 is optimal
                                       256,         // cache_size
                                       0,           // prive_data
                                       wanted_data_room,
                                       0); // socket_id
