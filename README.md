...
通報フェーズはg++で実行ファイルを生成して実行(詳細は下記の実行方法例)
ここで、common_func.cのincludeでDPDK_func.hをコメントアウトしTLS_func.hを用いるようにする
...
さらに、log_router_cycles関数を用いるとCSVファイル形式で任意の個数のサイクル数を保存できる
